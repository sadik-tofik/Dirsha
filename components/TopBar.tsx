...
import { ProfileIcon, BackArrowIcon } from './IconComponents';
    ...
    return (
        <header className="fixed top-0 left-0 right-0 bg-brand-dark/80 backdrop-blur-sm z-40 h-16 flex items-center px-4 shadow-lg shadow-purple-900/10">
            <div className="flex-1">
                {showBackButton && (
                    <Link to="/" className="flex items-center justify-center h-10 w-10 rounded-full bg-white/10 hover:bg-white/20 transition-colors" aria-label={t('back')}>
                        <BackArrowIcon className="w-6 h-6 text-brand-green" />
                    </Link>
                )}
            </div>
            <div className="flex items-center gap-2">
                <div className="relative">
                    <button 
                        ...
                        className="flex items-center justify-center rounded-full px-4 py-2 bg-white/10 hover:bg-white/20 transition-colors"
                        aria-label="Change language"
                    >
                        <span className="font-semibold text-sm uppercase text-gray-200">{language}</span>
                    </button>
                    {isLangDropdownOpen && (
